...
- [ ] **Banner Visuals:**
  ...
  - **Remaining Visualization:** Dynamic "Tu dinero valdrá:" column displaying remaining value.
...
- [ ] **Logic & Sync:**
  ...
  - The "Remaining Value" must be dynamically calculated as (`Current Savings` - `Estimated Loss`).
  - **Dynamic Legend:**
    - If years = 1: Display "en 12 meses".
    - If years > 1: Display "en [X] años".
- [ ] **Interaction Physics:**
  - Real-time recalculation when typing (600ms count-up transition for both values).
  - **Smooth Anchor CTA:** Single Emerald Green button ("Detén la Pérdida ↓") placed beneath both value columns, scrolling directly to the Lead Capture Form.
