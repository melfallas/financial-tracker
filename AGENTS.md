...
- **🏗️ Architectural Rule — JSON Formatting:** All API or Mock JSON definitions must strictly follow **snake_case** and all field names MUST be in **English**.
- **🏗️ Architectural Rule — Shared Simulator State:** Maintain a unified state in `SimulatorsStateService` where `initial_capital`, `monthly_contributions`, `term_in_years`, `annual_return`, and `annual_inflation` are shared across all simulators to ensure UX consistency.
- **🏗️ Architectural Rule — Input Validation:** Numeric inputs determining values can be null (empty). Navigation or calculation must be blocked with an error message if required inputs are empty (except for `monthly_contributions`, which can be 0).
- **📜 Architectural Rule — Documentation Consultation:** Consultants MUST read and follow `docs/Backlog.md`, `docs/PO_Checklist.md`, `docs/SprintReport.md`, `docs/Dependency-Analysis.md`, and `docs/PROJECT_INDEX.md` during the planning phase of any feature.
- **🔄 Architectural Rule — Documentation Consistency:** Any change requiring an update to project management artifacts or `PROJECT_INDEX.md` must be updated immediately to maintain synchronization.
